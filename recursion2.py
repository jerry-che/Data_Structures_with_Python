...
#  Description: Coding Bat recursion 2 problems
...
def groupSumClump(start, nums, target):
  if target == 0 and start >= len(nums):
    ...
    return False
  count_num = nums.count(nums[start])
  int_sum = nums[start] * count_num
  return groupSumClump(start + count_num, nums, target - int_sum) or groupSumClump(start + count_num,nums,target)
...
def splitArrayHelper(nums,idx,a,b):
  if idx >= len(nums) and a == b:
    return True
  elif idx >= len(nums):
    return False
  return splitArrayHelper(nums,idx + 1,a + nums[idx],b) or splitArrayHelper(nums,idx + 1,a,b + nums[idx])
...
def splitOdd10Helper(nums,idx,a,b):
  if idx >= len(nums) and  (((a % 10 == 0) and (b % 2 == 1)) or ((a % 2 == 1) and (b % 10 == 0))):
    return True
  elif idx >= len(nums):
    return False
  return splitOdd10Helper(nums,idx + 1, a + nums[idx],b) or splitOdd10Helper(nums,idx + 1,a,b + nums[idx])
...
def split53Helper(nums,idx,a,b):
  if idx >= len(nums) and a == b and (a != 0 and b !=0) :
    return True
  elif idx >= len(nums):
    return False
  elif nums[idx] % 3 == 0 and nums[idx] % 5 != 0:
    return split53Helper(nums, idx + 1,a, b + nums[idx])
  elif nums[idx] % 5 == 0:
    return split53Helper(nums,idx + 1,a + nums[idx],b)
  else:
    return split53Helper(nums,idx+1,a+ nums[idx],b) or split53Helper(nums,idx+1,a,b + nums[idx])
